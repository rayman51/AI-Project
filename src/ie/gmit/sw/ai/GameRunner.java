...
public class GameRunner implements KeyListener{
	...
	public static ControlledSprite player;
	...
	public GameRunner() throws Exception{
    	...
    	information.setText("  Health: " + Double.toString(player.getHealth()) + "   Sheild: " + player.getDurability()   
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_RIGHT && currentCol < MAZE_DIMENSION - 1) {
        	...
        	if(player.gethBomb() > 0){
        		player.sethBomb(player.gethBomb() - 1);
        		model.set(currentRow, currentCol, '\u0034');
        		(new Bomb('\u0031', model, currentRow, currentCol)).start();
        	}
        }else{
        	return;
        }
	...
	private boolean isValidMove(int row, int col){
		// Win clause here - If player exits the maze, the game exits
				if(isExitNode(row, col)){
					...
					}else if (model.get(currentRow, currentCol) == '\u0034'){
						model.set(currentRow, currentCol, '\u0034');
					}else{
						model.set(currentRow, currentCol, '\u0020');
					}
	...
	public boolean isExitNode(int row, int col){
		if(row == -1 || row == 100 || col == -1 || col == 100){
			return true;
		}// shud work.. will exit if set to zero when he hits outer wall
